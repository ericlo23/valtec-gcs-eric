...
3. No error logging with command context. Wrapped `command_service.execute` in a `try/except` to log execution errors. Uvicorn's built-in error logging does not include the command parameter details, which makes failures hard to trace and reproduce. The added logging records the command context (`type`, `payload`, `drone_id`) to aid troubleshooting. The exception is then re-raised (a bare `raise`, not converted to `HTTPException(500)`) so that Uvicorn still logs the full traceback at ERROR level. (Task 4 later made execution asynchronous, so this logging moved from the handler to the per-drone worker and no longer re-raises into a response.)
